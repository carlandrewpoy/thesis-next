...
import { forfetch } from "./publicationExportFetch"

export async function publicationExport(detailsData) {
  const fileName = `PUBLICATION_(${detailsData})`
  ...
  const handleExport = () => {
    const title = [
      {
        A: `Publication: Filtered by: Year(${detailsData})`,
      },
      {},
    ]
    ...
    data.forEach((row) => {
      tables.CCIS.push({
        ...
        I: row.researchers,
        J: row.journalTitle,
        K: row.publicationDate,
        L: row.issueNo,
        M: row.issnOrIsbn,
        N: row.index,
        O: row.supportingDocs,
      })
      forNo++
    })
    ...
    const dataInfo = {
      titleCell: "A2",
      titleRange: "A1:O2",
      tbodyRange: `A3:O${finalData.length}`,

      ////////////////aron d libogggggggggggg
      theadRangeHeader:
        headerIndexes?.length >= 1
          ? `A${headerIndexes[1] + 1}:O${headerIndexes[1] + 1}`
          : null,
      theadRange:
        headerIndexes?.length >= 1
          ? `A${headerIndexes[0] + 1}:O${headerIndexes[0] + 1}`
          : null,
