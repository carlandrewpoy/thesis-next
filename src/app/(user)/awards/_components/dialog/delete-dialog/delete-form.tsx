...
import { deletePublication } from '@/server-actions/publication'
import { deletePartnership } from '@/server-actions/partnerships'
import { AwardWithOthers } from '../../columns'
import { deleteAward } from '@/server-actions/award'

const DeleteForm = ({ row, close }: {
    row: Row<AwardWithOthers>
    close: Dispatch<SetStateAction<boolean>>
}) => {
    const [state, formAction] = useFormState(deleteAward, null)
    if (state?.message) {
        close(false)
        toast({
            duration: 1500,
